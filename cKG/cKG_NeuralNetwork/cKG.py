...
Updated at 03/23/2019:
    This function is designed for 10-dim Neural network function: 
        MLP_obj_c.py.
    Here are the difference between cKG for rosen/branin and Neural network.
    1. objective input change from 2-D to 10-D, 
        which is the most foundamental change.
    2. The GP coregionalized model dimension change.
    3. Initialized process need more times to run. Since 1 training point need
        one minute to run. Here we will try to start from 4 training points.
    4. The lhs need to change to 10-D.
    5. The calling of obj_func is much more time consuming, so the object 
        fD need cannot only save index, and it is not support vector input
        anymore.
    6. Saving the observe value for the recommendation directly since
        the function evaluation need long time.
    7. The objective function should be minimized. Since the return value is 
        correct rate, so we need to set objective to 1 - return value.
    8. When do the performance evaluation, remember to set objective to 
        minimized form.
...
def setup_model(fD, func, noise=1e-6):
    for ea in fD.keys():
        ...
        fD[ea].obs_val1 = np.array(fD[ea].obs_val)[:, None]
        fD[ea].obs_mean = np.mean(fD[ea].obs_val1)
        fD[ea].nmlz = fD[ea].obs_val1.std(0)
        fD[ea].Ny = (fD[ea].obs_val1 - fD[ea].obs_mean)/fD[ea].nmlz
    Ny = [fD['f'].Ny, fD['c1'].Ny]
...
def Get_min_cKG(fD, myPara, En_un_1_set, un_star, func):
    ...
    fD[min_task].obs_val.append(obj_func(spl_pt[:, 0:-1][0], min_task))
    fD[min_task].X = np.vstack([fD[min_task].X, spl_pt[:, 0:-1]])
    
    slc_n = 50
    tab_names = ["expec_un+1_f", "expec_un+1_c1"]
    tab_vars = [En_un_1_set['f'].val[0:slc_n, :] \
...
def get_un_star(fD, myPara, m, spl_num):
    ...
    obj_rec = obj_func(np.array(fD['f'].X_prd[ind]), f='f')
    cons_rec = obj_func(np.array(fD['c1'].X_prd[ind]), f='c1')
    util = (cons_rec <= 0.2)*obj_rec + (cons_rec > 0.2)
    myPara.rec_util.append(util)
    myPara.obj_rec.append(obj_rec)
    myPara.cons_rec.append(cons_rec)
    col_names = ["un_star", "condi_expec_mean", "post_mean", \
                 "post_var", "feasi_prob", "true_f_val", "true_c_val"]
    tab_var = [un_star, E_n_condi[ind, 0], fD['f'].mean_prd[ind, 0], \
               fD['f'].var_prd[ind, 0], Pr_feasible[ind, 0], 'N/A', 'N/A']
    tab = get_tab(col_names, tab_var)
    logger.info("\n%s", tab)
    col_names = ["f_predict_mean", "E_n_condi", \
                 "f_predict_var", "un_set", "feasiblity"]
    slc_n = 50
    tab_var = [fD['f'].mean_prd[0:slc_n, :], \
...
class Func_Dict(object):
    ...
    def __init__(self, X_prd, num_train, task, fname):        
        ...
        self.noise_dict = {'output_index':self.X_prd[:, -1].astype(int)}
        self.obs_val = []
        for i in range(self.X.shape[0]):
            self.obs_val.append(obj_func(self.X[i, :], task))
...
def predict_chol(m, X_new, noise=1e-6):
    ...
    Myvar = (Kxx - np.square(tmp).sum(0))[:, None] + noise
    return mean, Myvar
def obj_func(params, f='f'):
    '''
    The objtive and constraint function for cKG:
    Neural Network error and the prediciton time.
    '''
    if f=='f':
        out = 1 - run_obj(params)
    else:
        out = run_cons(params)
    return out
